...
func (es eventStore) ShareThing(ctx context.Context, token, thingID string, actions []string, userID string) error {
	return es.svc.ShareThing(ctx, token, thingID, actions, userID)
}
