...
func (lm *loggingMiddleware) ShareThing(ctx context.Context, token, thingID string, actions []string, userID string) (err error) {
	defer func(begin time.Time) {
	...
	return lm.svc.ShareThing(ctx, token, thingID, actions, userID)
}
