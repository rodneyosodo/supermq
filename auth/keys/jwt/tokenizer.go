...
func (c claims) Valid() error {
	if c.Type == nil || *c.Type > keys.APIKey || c.Issuer != issuerName {
...
func New(secret string) keys.Tokenizer {
	return tokenizer{secret: secret}
}

func (svc tokenizer) Issue(key keys.Key) (string, error) {
	claims := claims{
...
func (svc tokenizer) Parse(token string) (keys.Key, error) {
	c := claims{}
	...
	if err != nil {
		if e, ok := err.(*jwt.ValidationError); ok && e.Errors == jwt.ValidationErrorExpired {
			...
			if c.Type != nil && *c.Type == keys.APIKey {
				return c.toKey(), keys.ErrAPIKeyExpired
			}
			return keys.Key{}, errors.Wrap(keys.ErrKeyExpired, err)
		}
		return keys.Key{}, errors.Wrap(errors.ErrAuthentication, err)
	}

	return c.toKey(), nil
}

func (c claims) toKey() keys.Key {
	key := keys.Key{
		ID:       c.Id,
		IssuerID: c.IssuerID,
		Subject:  c.Subject,
