...
var _ keys.KeyRepository = (*repo)(nil)
...
func New(db postgres.Database) keys.KeyRepository {
	return &repo{
		db: db,
	}
}

func (kr repo) Save(ctx context.Context, key keys.Key) (string, error) {
...
func (kr repo) Retrieve(ctx context.Context, issuerID, id string) (keys.Key, error) {
	...
	if err := kr.db.QueryRowxContext(ctx, q, issuerID, id).StructScan(&key); err != nil {
		...
		if err == sql.ErrNoRows || ok && pgerrcode.InvalidTextRepresentation == pgErr.Code {
			return keys.Key{}, errors.Wrap(errors.ErrNotFound, err)
		}

		return keys.Key{}, errors.Wrap(errRetrieve, err)
	}
...
func toDBKey(key keys.Key) dbKey {
	ret := dbKey{
		ID:       key.ID,
		Type:     key.Type,
