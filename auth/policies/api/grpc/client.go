...
func NewClient(tracer opentracing.Tracer, conn *grpc.ClientConn, timeout time.Duration) mainflux.AuthServiceClient {
	return &grpcClient{
		...
		assign: otelkit.EndpointMiddleware(otelkit.WithOperation("assign"))(kitgrpc.NewClient(
			conn,
			svcName,
			"Assign",
			encodeAssignRequest,
		...
		members: otelkit.EndpointMiddleware(otelkit.WithOperation("members"))(kitgrpc.NewClient(
			conn,
			svcName,
			"Members",
