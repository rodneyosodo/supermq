...
func TestClientsMemberships(t *testing.T) {
	...
	userA := users.User{
		...
		Credentials: users.Credentials{
			Identity: "user-memberships1@example.com",
			Secret:   password,
		},
	...
	userB := users.User{
		...
		Name: "user-memberships",
		Credentials: users.Credentials{
			Identity: "user-memberships2@example.com",
			Secret:   password,
		},
		Metadata: users.Metadata{},
		Status:   users.EnabledStatus,
	...
	_, err := crepo.Save(context.Background(), userA)
	assert.True(t, errors.Contains(err, nil), fmt.Sprintf("save user: expected %v got %s\n", nil, err))
	_, err = crepo.Save(context.Background(), userB)
	assert.True(t, errors.Contains(err, nil), fmt.Sprintf("save user: expected %v got %s\n", nil, err))
	...
	cases := map[string]struct {
		...
		"retrieve membership for existing user":     {userA.ID, nil},
		"retrieve membership for non-existing user": {wrongID, nil},
	}

	for desc, tc := range cases {
		mp, err := grepo.Memberships(context.Background(), tc.ID, groups.GroupsPage{Page: groups.Page{Total: 10, Offset: 0, Limit: 10, Status: groups.AllStatus, Subject: userB.ID, Action: "g_list"}})
		assert.True(t, errors.Contains(err, tc.err), fmt.Sprintf("%s: expected %s got %s\n", desc, tc.err, err))
		if tc.ID == userA.ID {
