...
func main() {
	...
	defer pubsub.Close()
	pubsub = brokerstracing.NewPubSub(serverConfig, tracer, pubsub)

	pubsub, err = msgevents.NewPubSubMiddleware(ctx, pubsub, cfg.ESURL)
	if err != nil {
	...
	h := mqtt.NewHandler(pubsub, mpub, logger, clientsClient, channelsClient)
