...
func (ms *metricsMiddleware) ListUsers(ctx context.Context, token string, active bool, offset, limit uint64, email string, um users.Metadata) (users.UserPage, error) {
	defer func(begin time.Time) {
		ms.counter.With("method", "list_users").Add(1)
		ms.latency.With("method", "list_users").Observe(time.Since(begin).Seconds())
	}(time.Now())

	return ms.svc.ListUsers(ctx, token, active, offset, limit, email, um)
}
...
func (ms *metricsMiddleware) ListMembers(ctx context.Context, token, groupID string, active bool, offset, limit uint64, gm users.Metadata) (users.UserPage, error) {
	defer func(begin time.Time) {
		ms.counter.With("method", "list_members").Add(1)
		ms.latency.With("method", "list_members").Observe(time.Since(begin).Seconds())
	}(time.Now())

	return ms.svc.ListMembers(ctx, token, groupID, active, offset, limit, gm)
}

func (ms *metricsMiddleware) RemoveUser(ctx context.Context, token string, id string) (err error) {
	defer func(begin time.Time) {
		ms.counter.With("method", "remove_user").Add(1)
		ms.latency.With("method", "remove_user").Observe(time.Since(begin).Seconds())
	}(time.Now())

	return ms.svc.RemoveUser(ctx, token, id)
}
